...
#Make sure script is run from 'auv01' user account
if [ "$USER" != "auv01" ];then
	echo "Run script from 'auv01' user account"
	exit -1
fi
...
#get companion repo
get_repo "$HOME/companion" "https://github.com/cal852/companion"
...
if [ "$1" != "update" ]; then
	...
	S1="sleep 10"
	S2="sudo -H -u auv01 /bin/bash -c '/home/auv01/companion/scripts/autostart_mavproxy.sh'"
	S3="sudo -H -u auv01 /bin/bash -c '/home/auv01/companion/scripts/autostart_gstreamer.sh'"
	...
	sudo sh -c "echo 'SUBSYSTEM==\"tty\", ATTRS{idVendor}==\"0483\", ATTRS{idProduct}==\"5740\", SYMLINK+=\"pixhawk\"' > /etc/udev/rules.d/99-usb-serial.rules"
